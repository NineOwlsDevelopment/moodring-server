...
const PRECISION = new Decimal(1_000_000);
const LN2_SCALED = new Decimal("0.6931471805599453"); // ln(2) with high precision
 ...
 */
function toDecimal(value: number | bigint | Decimal | any): Decimal {
...
export function calculate_buy_cost(
  current_yes: number | bigint | Decimal | any,
  current_no: number | bigint | Decimal | any,
  buy_yes: number | bigint | Decimal | any,
  buy_no: number | bigint | Decimal | any,
  liquidity_param: number | bigint | Decimal | any
): number {
  ...
  if (result.isZero() && (!b_yes.isZero() || !b_no.isZero())) {
    throw new InvariantViolationError(
      "Cost should be positive for non-zero purchase"
    );
  }
...
export function calculate_sell_payout(
  current_yes: number | bigint | Decimal | any,
  current_no: number | bigint | Decimal | any,
  sell_yes: number | bigint | Decimal | any,
  sell_no: number | bigint | Decimal | any,
  liquidity_param: number | bigint | Decimal | any
): number {
  const q_yes = toDecimal(current_yes);
  const q_no = toDecimal(current_no);
  ...
  if (result.isZero() && (!s_yes.isZero() || !s_no.isZero())) {
    throw new InvariantViolationError(
      "Payout should be positive for non-zero sale"
    );
  }
...
export function calculate_yes_price(
  yes_quantity: number | bigint | Decimal | any,
  no_quantity: number | bigint | Decimal | any,
  liquidity_param: number | bigint | Decimal | any
): number {
  ...
  if (yes_price.isNegative()) {
    throw new InvariantViolationError(
      `YES price cannot be negative: ${yes_price.toString()}`
    );
  }
  if (yes_price.gt(PRECISION)) {
    throw new InvariantViolationError(
      `YES price cannot exceed PRECISION: ${yes_price.toString()} > ${PRECISION.toString()}`
    );
  }
...
export function calculate_no_price(
  ...
  const yes_price_decimal = toDecimal(
    calculate_yes_price(yes_quantity, no_quantity, liquidity_param)
  );
  ...
  if (diff.gt(new Decimal(1))) {
    throw new InvariantViolationError(
      `Price sum not equal to PRECISION: yes=${yes_price_decimal.toString()}, no=${no_price.toString()}, sum=${sum.toString()}`
    );
  }
...
function cost_function(
  ...
  const one_plus_exp = new Decimal(1).add(exp_neg);
  ...
  if (result.lt(max_q)) {
    throw new InvariantViolationError(
      `Cost function violated: C(${yes_quantity}, ${no_quantity}) < max(${yes_quantity}, ${no_quantity})`
    );
  }
...
// Export PRECISION and error classes for use in tests
export { PRECISION, InvariantViolationError };

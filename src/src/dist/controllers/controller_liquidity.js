...
exports.claimLpRewards = exports.calculateLpShareValue = exports.getAllLpPositions = exports.getLpPosition = exports.removeLiquidity = exports.addLiquidity = void 0;
 ...
 */
function calculateLpShares(amount, poolLiquidity, totalShares) {
    ...
    const MIN_DEPOSIT = BigInt(1000000);
    if (amountBigInt < MIN_DEPOSIT) {
        throw new Error(`Minimum deposit is ${MIN_DEPOSIT / BigInt(1000000)} USDC to prevent rounding exploits`);
    }
    ...
    if (totalSharesBigInt === 0n || poolLiquidityBigInt === 0n) {
        ...
        if (amountBigInt < MIN_DEPOSIT) {
            throw new Error(`Minimum deposit is ${MIN_DEPOSIT / BigInt(1000000)} USDC`);
        }
 ...
 */
function calculateShareValue(shares, poolLiquidity, accumulatedFees, totalShares) {
    if (totalShares === 0)
        return 0;
 ...
 */
async function calculateOutstandingLiability(client, marketId) {
    // Get total outstanding shares across all options
    const sharesResult = await client.query(`SELECT COALESCE(SUM(yes_quantity + no_quantity), 0)::bigint as total_shares
     FROM market_options 
 ...
 */
async function calculatePendingClaims(client, marketId) {
    // Get all resolved options for this market
    const optionsResult = await client.query(`SELECT id, winning_side 
     FROM market_options 
     WHERE market_id = $1 AND is_resolved = TRUE AND winning_side IS NOT NULL`, [marketId]);
    ...
    for (const option of optionsResult.rows) {
        ...
        const positionsResult = await client.query(`SELECT yes_shares, no_shares 
       FROM user_positions 
       WHERE option_id = $1 
         AND is_claimed = FALSE 
         AND (yes_shares > 0 OR no_shares > 0)`, [optionId]);
...
const addLiquidity = async (req, res) => {
    try {
        ...
        const amountValidation = (0, validation_1.validateNumber)(amount, "Amount", 0.000001, undefined);
        ...
        const result = await (0, tradeQueue_1.withTradeQueue)(marketId, undefined, async () => {
            return await (0, transaction_1.withTransaction)(async (client) => {
                ...
                const marketResult = await client.query(`SELECT 
                *,
               ...
               FROM markets WHERE id = $1 FOR UPDATE`, [marketId]);
                ...
                if (market.is_resolved) {
                    throw new transaction_1.TransactionError(400, "Cannot add liquidity to resolved market");
                }
                ...
                const currentPoolLiquidity = Number(market.shared_pool_liquidity) || 0;
                const currentTotalShares = Number(market.total_shared_lp_shares) || 0;
                // Debug: Log raw values from database
                console.log("=== Raw Database Values ===");
                console.log("market.shared_pool_liquidity (raw):", market.shared_pool_liquidity, typeof market.shared_pool_liquidity);
                console.log("market.total_shared_lp_shares (raw):", market.total_shared_lp_shares, typeof market.total_shared_lp_shares);
                console.log("currentPoolLiquidity (processed):", currentPoolLiquidity);
                console.log("currentTotalShares (processed):", currentTotalShares);
                // Calculate LP shares to mint (6 decimal precision)
                const sharesToMint = calculateLpShares(parsedAmount, currentPoolLiquidity, currentTotalShares);
                console.log("=== Add Liquidity Calculation ===");
                console.log("parsedAmount (micro-USDC):", parsedAmount);
                console.log("currentPoolLiquidity (micro-USDC):", currentPoolLiquidity);
                console.log("currentTotalShares (micro-units):", currentTotalShares);
                console.log("sharesToMint (micro-units):", sharesToMint);
                console.log("Ratio:", currentTotalShares > 0 && currentPoolLiquidity > 0
                    ? `${(sharesToMint / parsedAmount).toFixed(6)}:1`
                    : "1:1 (first LP)");
                console.log("Calculation:", currentTotalShares > 0 && currentPoolLiquidity > 0
                    ? `Math.floor((${parsedAmount} * ${currentTotalShares}) / ${currentPoolLiquidity}) = ${sharesToMint}`
                    : `First LP: ${parsedAmount} (1:1)`);
                if (sharesToMint <= 0) {
                    throw new transaction_1.TransactionError(400, "Amount too small to mint shares");
                }
               ...
               FROM market_options WHERE market_id = $1`, [marketId]);
                ...
                const newLiquidityParam = Math.max(Number(market.base_liquidity_parameter) * 1000, Math.floor(Math.sqrt(marketSize) * 10000));
                await client.query(`UPDATE markets SET 
          shared_pool_liquidity = $1,
          total_shared_lp_shares = $2,
          liquidity_parameter = $3,
          updated_at = EXTRACT(EPOCH FROM NOW())::BIGINT
         WHERE id = $4`, [newLiquidity, newTotalShares, newLiquidityParam, marketId]);
                // Verify the update was successful
                const verifyResult = await client.query(`SELECT shared_pool_liquidity, total_shared_lp_shares FROM markets WHERE id = $1`, [marketId]);
                const verified = verifyResult.rows[0];
                console.log("=== Verification After Update ===");
                console.log("Verified shared_pool_liquidity:", verified?.shared_pool_liquidity);
                console.log("Verified total_shared_lp_shares:", verified?.total_shared_lp_shares);
...
const removeLiquidity = async (req, res) => {
    try {
        ...
        const sharesValidation = (0, validation_1.validateNumber)(shares, "Shares", 0.000001, undefined);
        if (!sharesValidation.isValid) {
            return (0, errors_1.sendValidationError)(res, sharesValidation.error);
        }
        ...
        const result = await (0, tradeQueue_1.withTradeQueue)(marketId, undefined, async () => {
            return await (0, transaction_1.withTransaction)(async (client) => {
                ...
                const positionResult = await client.query(`SELECT * FROM lp_positions WHERE user_id = $1 AND market_id = $2 FOR UPDATE`, [userId, marketId]);
                const position = positionResult.rows[0];
                if (!position || Number(position.shares) < parsedShares) {
                    throw new transaction_1.TransactionError(400, "Insufficient LP shares");
                }
                // Lock LP until market is resolved
                if (!market.is_resolved) {
                    throw new transaction_1.TransactionError(400, "Cannot remove liquidity until market is resolved. LP shares are locked until resolution.");
                }
                ...
                if (totalShares <= 0) {
                    throw new transaction_1.TransactionError(400, "Pool has no shares");
                }
                ...
                const pendingClaims = await calculatePendingClaims(client, marketId);
                ...
                const availableLiquidity = Math.max(0, poolLiquidity - pendingClaims - reservedLiquidity);
                ...
                const liquidityPortion = Math.floor(availableLiquidity * shareRatio);
                ...
                if (newReservedLiquidity > poolLiquidity - pendingClaims) {
                    throw new transaction_1.TransactionError(400, "Insufficient available liquidity. Another withdrawal may be in progress.");
                }
                // Update reserved liquidity atomically
                await client.query(`UPDATE markets SET reserved_liquidity = $1, updated_at = EXTRACT(EPOCH FROM NOW())::BIGINT WHERE id = $2`, [newReservedLiquidity, marketId]);
                if (usdcToReturn <= 0) {
                    throw new transaction_1.TransactionError(400, "No claimable amount available. All liquidity is reserved for pending trader claims.");
                }
                // Safety check: ensure we're not withdrawing more liquidity than available
                if (liquidityPortion > availableLiquidity) {
                    throw new transaction_1.TransactionError(400, `Invalid withdrawal: requested ${(liquidityPortion / 1000000).toFixed(2)} USDC from pool but only ${(availableLiquidity / 1000000).toFixed(2)} USDC available.`);
                }
                // Safety check: ensure we're not claiming more fees than exist
                if (feesPortion > accumulatedFees) {
                    throw new transaction_1.TransactionError(400, `Invalid fees claim: requested ${(feesPortion / 1000000).toFixed(2)} USDC in fees but only ${(accumulatedFees / 1000000).toFixed(2)} USDC available.`);
                }
                // Update LP position
                const newShares = Number(position.shares) - parsedShares;
                // Use BigInt for precision: (deposited_amount * newShares) / shares
                const newDepositedAmount = Number((BigInt(position.deposited_amount) * BigInt(newShares)) /
                    BigInt(position.shares));
                const newTokenBalance = Math.max(0, Number(position.lp_token_balance || 0) - parsedShares);
                if (newShares <= 0) {
                    await client.query(`DELETE FROM lp_positions WHERE user_id = $1 AND market_id = $2`, [userId, marketId]);
                }
                else {
                    await client.query(`UPDATE lp_positions 
           ...
           WHERE user_id = $1 AND market_id = $2`, [
                ...
                const newAccumulatedFees = Math.max(0, accumulatedFees - feesPortion);
                const newTotalShares = totalShares - parsedShares;
                // SECURITY FIX (CVE-002): Release reserved liquidity after withdrawal completes
                const finalReservedLiquidity = Math.max(0, newReservedLiquidity - liquidityPortion);
                // Get total shares across all options to scale liquidity parameter
                const totalSharesResult = await client.query(`SELECT COALESCE(SUM(yes_quantity + no_quantity), 0)::bigint as total_shares
               FROM market_options WHERE market_id = $1`, [marketId]);
                const totalOptionShares = Number(totalSharesResult.rows[0]?.total_shares || 0);
                ...
                // Formula: b = max(base_param * 1000, sqrt(max(liquidity, total_shares)) * 10000)
                // This ensures b scales with both liquidity provision AND trading volume
                const newLiquidityParam = newPoolLiquidity > 0
                    ? (() => {
                        const marketSize = Math.max(newPoolLiquidity, totalOptionShares);
                        return Math.max(Number(market.base_liquidity_parameter) * 1000, Math.floor(Math.sqrt(marketSize) * 10000));
                    })()
         ...
         WHERE id = $6`, [
                ...
                // Credit user wallet
                await client.query(`UPDATE wallets SET balance_usdc = balance_usdc + $1, updated_at = EXTRACT(EPOCH FROM NOW())::BIGINT WHERE user_id = $2`, [usdcToReturn, userId]);
                return {
                    usdcToReturn,
                    feesPortion,
                    newPoolLiquidity,
 ...
 * @access Private
 */
const getLpPosition = async (req, res) => {
    try {
        const userId = req.id;
        ...
        const position = await LpPosition_1.LpPositionModel.findByUserAndMarket(userId, market);
        if (!position) {
            return (0, errors_1.sendSuccess)(res, {
                position: {
                    shares: 0,
        ...
        const marketData = await Market_1.MarketModel.findById(market);
        let currentValue = 0;
        ...
        if (marketData && Number(marketData.total_shared_lp_shares) > 0) {
            ...
            currentValue = calculateShareValue(userShares, poolLiquidity, accumulatedFees, totalShares);
            ...
            if (marketData.is_resolved) {
                ...
                try {
                    ...
                    const claimableLiquidityPortion = Math.floor(availableLiquidity * shareRatio);
                    const feesPortion = Math.floor(accumulatedFees * shareRatio);
...
exports.getLpPosition = getLpPosition;
/**
 * @route GET /api/liquidity/positions
 * @desc Get all LP positions for a user
 * @access Private
 */
const getAllLpPositions = async (req, res) => {
    try {
        const userId = req.id;
        if (!userId) {
            return (0, errors_1.sendError)(res, 401, "Unauthorized");
        }
        const positions = await LpPosition_1.LpPositionModel.findByUser(userId);
        // Get market data for all positions
        const marketIds = positions.map((p) => p.market_id);
        const marketsResult = await db_1.pool.query(`SELECT id, question, shared_pool_liquidity, accumulated_lp_fees, total_shared_lp_shares, is_resolved
       FROM markets WHERE id = ANY($1::uuid[])`, [marketIds]);
        ...
        const positionsWithValue = positions.map((position) => {
            ...
            if (market && Number(market.total_shared_lp_shares) > 0) {
                currentValue = calculateShareValue(Number(position.shares), Number(market.shared_pool_liquidity), Number(market.accumulated_lp_fees), Number(market.total_shared_lp_shares));
            }
            return {
                ...position,
                shares: Number(position.shares),
        ...
        return (0, errors_1.sendSuccess)(res, { positions: positionsWithValue });
    }
    catch (error) {
...
const calculateLpShareValue = async (req, res) => {
    try {
        ...
        const totalPoolValue = Number(marketData.shared_pool_liquidity) +
            Number(marketData.accumulated_lp_fees);
        const totalShares = Number(marketData.total_shared_lp_shares);
        const valuePerShare = totalShares > 0 ? totalPoolValue / totalShares : 1;
        const lpCount = await LpPosition_1.LpPositionModel.getMarketLpCount(market);
        return (0, errors_1.sendSuccess)(res, {
            ...
            lp_count: lpCount,
            lp_token_mint: marketData.lp_token_mint || null,
        });
    }
    catch (error) {
    ...
    try {
        ...
        const result = await (0, tradeQueue_1.withTradeQueue)(marketId, undefined, async () => {
            return await (0, transaction_1.withTransaction)(async (client) => {
                ...
                if (!market.is_resolved) {
                    throw new transaction_1.TransactionError(400, "Market is not resolved. Use /remove endpoint for early withdrawal.");
                }
                // Get LP position with lock SECOND (consistent locking order)
                const positionResult = await client.query(`SELECT * FROM lp_positions WHERE user_id = $1 AND market_id = $2 FOR UPDATE`, [userId, marketId]);
                const position = positionResult.rows[0];
                if (!position || Number(position.shares) <= 0) {
                    throw new transaction_1.TransactionError(400, "No LP shares to claim");
                }
                ...
                const sharesToClaim = requestedShares
                    ...
                    : userShares;
                if (sharesToClaim <= 0) {
                    throw new transaction_1.TransactionError(400, "Invalid share amount");
                }
                ...
                if (payout > maxPayout) {
                    throw new transaction_1.TransactionError(400, `Invalid claim amount: requested ${(payout / 1000000).toFixed(2)} USDC but only ${(maxPayout / 1000000).toFixed(2)} USDC available.`);
                }
                // Additional safety: ensure we're not claiming more liquidity than exists
                if (liquidityPortion > availableLiquidity) {
                    throw new transaction_1.TransactionError(400, `Invalid liquidity claim: requested ${(liquidityPortion / 1000000).toFixed(2)} USDC from pool but only ${(availableLiquidity / 1000000).toFixed(2)} USDC available.`);
                }
                ...
                // Calculate breakdown
                const proportionalDeposit = Math.floor((depositedAmount * sharesToClaim) / userShares);
                const pnl = payout - proportionalDeposit;
                // Calculate new pool liquidity after withdrawal
                const newPoolLiquidity = Math.max(0, poolLiquidity - liquidityPortion);
                // SECURITY FIX (CVE-002): Release reserved liquidity after withdrawal completes
                const finalReservedLiquidity = Math.max(0, newReservedLiquidity - liquidityPortion);
                ...
                if (newPoolLiquidity < pendingClaims) {
                    const pendingClaimsFormatted = (pendingClaims / 1000000).toFixed(2);
                    const remainingFormatted = (newPoolLiquidity / 1000000).toFixed(2);
                    throw new transaction_1.TransactionError(400, `Cannot claim LP rewards: ${pendingClaimsFormatted} USDC needed for pending claims, but only ${remainingFormatted} USDC would remain after withdrawal. Please wait until all claims are processed.`);
                }
                // Update LP position
                const remainingShares = userShares - sharesToClaim;
                const remainingDeposit = depositedAmount - proportionalDeposit;
                const remainingTokenBalance = Math.max(0, lpTokenBalance - sharesToClaim);
         ...
         WHERE id = $5`, [
                ...
                // Credit user wallet
                await client.query(`UPDATE wallets SET balance_usdc = balance_usdc + $1, updated_at = EXTRACT(EPOCH FROM NOW())::BIGINT WHERE user_id = $2`, [payout, userId]);
...
exports.claimLpRewards = claimLpRewards;
//# sourceMappingURL=controller_liquidity.js.map
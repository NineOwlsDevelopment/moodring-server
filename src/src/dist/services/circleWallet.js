...
exports.CircleWalletService = exports.getCircleWallet = exports.initializeCircleWallet = void 0;
...
class CircleWalletService {
    ...
    async initialize() {
        try {
            const apiKey = await secrets_1.secretsManager.getRequiredSecret("CIRCLE_API_KEY");
            const entitySecret = await secrets_1.secretsManager.getRequiredSecret("CIRCLE_ENTITY_SECRET");
            let walletSetId = process.env.CIRCLE_WALLET_SET_ID;
            if (!apiKey || !entitySecret) {
                console.warn("[CircleWallet] CIRCLE_API_KEY or CIRCLE_ENTITY_SECRET not set. Circle wallet features disabled.");
                return false;
            }
            try {
    ...
    async getOrCreateWalletSet() {
        ...
        try {
            ...
            console.error("[CircleWallet] Failed to create wallet set:", JSON.stringify(errorDetails, null, 2));
    ...
    async createUserWallet(userId) {
        try {
            ...
            const response = await this.client.createWallets({
                accountType: "EOA",
                blockchains: [process.env.CIRCLE_BLOCKCHAIN || "SOL-DEVNET"],
                count: 1,
            ...
            console.log(`[CircleWallet] Created wallet for user ${userId}: ${wallet.id}`);
            return {
                walletId: wallet.id,
            ...
            const errorDetails = error.response?.data || error.data || error.message;
            console.error("[CircleWallet] Failed to create user wallet:", JSON.stringify(errorDetails, null, 2));
            // Provide more helpful error message
            const errorMessage = error.response?.data?.message ||
                error.response?.data?.error ||
                error.message ||
    ...
    async createHotWallet(name) {
        try {
            ...
            console.log(`[CircleWallet] Created hot wallet${name ? ` (${name})` : ""}: ${wallet.id}, address: ${wallet.address}`);
            return {
                walletId: wallet.id,
                address: wallet.address || "",
            };
            ...
            // Log full error details for debugging
            const errorDetails = error.response?.data || error.data || error.message;
            console.error("[CircleWallet] Failed to create hot wallet:", JSON.stringify(errorDetails, null, 2));
            // Provide more helpful error message
            const errorMessage = error.response?.data?.message ||
                error.response?.data?.error ||
                error.message ||
    ...
    async getUsdcBalance(walletId) {
        ...
        try {
            ...
            const usdcToken = tokenBalances.data?.tokenBalances?.find((token) => token.token?.tokenAddress === (0, constants_1.getUsdcMintAddress)(process.env.RPC_URL));
            // If token not found or amount is undefined, return 0
            if (!usdcToken ||
                usdcToken.amount === undefined ||
                usdcToken.amount === null) {
                console.log(`[CircleWallet] getUsdcBalance - No USDC token found or amount is null/undefined for wallet ${walletId}`);
                return 0;
            }
            ...
            if (isNaN(amount) || !isFinite(amount)) {
                console.warn(`[CircleWallet] Invalid USDC amount for wallet ${walletId}: ${usdcToken.amount}`);
                return 0;
            }
            ...
            console.error("[CircleWallet] Failed to get USDC balance:", error.message);
            return 0;
        }
    }
    /**
    ...
    async getSolBalance(walletId) {
        ...
        try {
            ...
            const solBalance = balances.find((b) => b.token?.symbol === "SOL" || !b.token);
            if (!solBalance) {
                return 0;
            }
    ...
    async getTransactionHash(transactionId) {
        ...
        try {
            ...
            console.error(`[CircleWallet] Failed to get transaction hash for ${transactionId}:`, error.message);
            return null;
        }
    }
    /**
     ...
     */
    async sendUsdc(walletId, destinationAddress, amount) {
        if (!this.client) {
            throw new Error("Circle wallet client not initialized");
        }
        console.log(`[CircleWallet] sendUsdc - Received amount: ${amount} (type: ${typeof amount}, isNaN: ${isNaN(amount)}, isFinite: ${isFinite(amount)})`);
        // Validate amount is a valid number
        if (isNaN(amount) || !isFinite(amount)) {
            console.error(`[CircleWallet] sendUsdc - Invalid amount detected: ${amount}`);
        ...
        console.log(`[CircleWallet] sendUsdc - Converted to string: "${amountString}"`);
        try {
            ...
            console.log(`[CircleWallet] Creating transaction: amount=${amountString} USDC, from=${walletId}, to=${destinationAddress}`);
            // Set hot wallet as fee payer if available
            if (hotWalletInfo?.address) {
                transactionParams.feePayer = hotWalletInfo.address;
                console.log(`[CircleWallet] Using hot wallet ${hotWalletInfo.address} as fee payer`);
            }
            ...
            console.log(`[CircleWallet] Created USDC transfer transaction: ${transactionId}`);
            ...
            if (typeof errorMessage === "string" &&
                ...
                const paymasterError = new Error(`Paymaster policy not configured for mainnet. Please set up a paymaster policy in the Circle developer console before sending transactions on mainnet. Original error: ${errorMessage}`);
                console.error("[CircleWallet] Paymaster policy error:", paymasterError.message);
     ...
     * @returns Transaction ID
     */
    async sendSol(walletId, destinationAddress, amount) {
        if (!this.client) {
            throw new Error("Circle wallet client not initialized");
        }
        if (amount <= 0) {
        ...
        try {
            ...
            console.log(`[CircleWallet] Created SOL transfer transaction: ${transactionId}`);
            await this.waitForTransaction(transactionId);
            return transactionId;
        }
        catch (error) {
    ...
    async waitForTransaction(transactionId, maxWaitMs = 60000) {
        ...
        while (Date.now() - startTime < maxWaitMs) {
            try {
                ...
                if (state === "DENIED" || state === "CANCELLED") {
                    throw new Error(`Transaction ${transactionId} failed with state: ${state}`);
                }
        ...
        throw new Error(`Transaction ${transactionId} did not confirm within ${maxWaitMs}ms`);
    }
    /**
    ...
    async getTransactionStatus(transactionId) {
        ...
        try {
            ...
            console.error("[CircleWallet] Failed to get transaction status:", error.message);
            return "ERROR";
        }
    }
    /**
    ...
    async getHotWalletInfo() {
        ...
        if (!hotWalletId) {
            console.warn("[CircleWallet] CIRCLE_HOT_WALLET_ID not set. Hot wallet sweep disabled.");
            return null;
        }
        try {
            ...
            if (!address) {
                console.error("[CircleWallet] Hot wallet address not found for wallet ID:", hotWalletId);
                return null;
            }
            return {
            ...
            console.error("[CircleWallet] Failed to get hot wallet info:", error.message);
            return null;
        }
    }
    /**
     ...
     */
    async sweepUsdcToHotWallet(userWalletId, amount) {
        if (!this.client) {
            console.error("[CircleWallet] Cannot sweep - Circle wallet not initialized");
            return null;
        }
        try {
            // Get hot wallet info
            const hotWalletInfo = await this.getHotWalletInfo();
            if (!hotWalletInfo) {
                console.error("[CircleWallet] Cannot sweep - hot wallet not configured");
                return null;
            }
            // Get user wallet balance - we'll sweep the entire balance
            // This ensures any previously missed deposits are also swept
            console.log(`[CircleWallet] sweepUsdcToHotWallet - Getting balance for wallet ${userWalletId} (deposit amount: ${amount} micro-USDC)`);
            const userBalance = await this.getUsdcBalance(userWalletId);
            console.log(`[CircleWallet] sweepUsdcToHotWallet - userBalance: ${userBalance} (type: ${typeof userBalance}, isNaN: ${isNaN(userBalance)}, isFinite: ${isFinite(userBalance)})`);
            // Validate balance is a valid number
            if (isNaN(userBalance) || !isFinite(userBalance)) {
                console.error(`[CircleWallet] Invalid balance for wallet ${userWalletId}: ${userBalance}`);
                return null;
            }
            if (userBalance <= 0) {
                console.warn(`[CircleWallet] No balance to sweep for wallet ${userWalletId}. Balance: ${userBalance} micro-USDC`);
                return null;
            }
            // Sweep the entire balance (not just the deposit amount)
            // Convert from micro-USDC to USDC (Circle API expects base units)
            const divisor = 1000000;
            console.log(`[CircleWallet] sweepUsdcToHotWallet - Dividing ${userBalance} by ${divisor}`);
            const usdcAmount = userBalance / divisor;
            console.log(`[CircleWallet] sweepUsdcToHotWallet - usdcAmount: ${usdcAmount} (type: ${typeof usdcAmount}, isNaN: ${isNaN(usdcAmount)}, isFinite: ${isFinite(usdcAmount)})`);
            // Validate converted amount is still a valid number
            if (isNaN(usdcAmount) || !isFinite(usdcAmount)) {
                console.error(`[CircleWallet] Invalid converted USDC amount for wallet ${userWalletId}: ${usdcAmount} (from balance: ${userBalance} micro-USDC)`);
                return null;
            }
            console.log(`[CircleWallet] Sweeping full balance: ${userBalance} micro-USDC (${usdcAmount} USDC) from wallet ${userWalletId} to hot wallet (deposit that triggered sweep: ${amount} micro-USDC)`);
            console.log(`[CircleWallet] sweepUsdcToHotWallet - Calling sendUsdc with amount: ${usdcAmount} (type: ${typeof usdcAmount})`);
            const transactionId = await this.sendUsdc(userWalletId, hotWalletInfo.address, usdcAmount);
            console.log(`[CircleWallet] ✅ Swept ${userBalance} micro-USDC (${userBalance / 1000000} USDC) from wallet ${userWalletId} to hot wallet ${hotWalletInfo.walletId} - tx: ${transactionId}`);
            return transactionId;
        }
        catch (error) {
            // Extract error message from Circle API response
            ...
            if (typeof errorMessage === "string" &&
                errorMessage.includes("paymaster policy")) {
                console.error("[CircleWallet] ❌ Failed to sweep USDC - Paymaster policy not configured for mainnet. Please set up a paymaster policy in the Circle developer console. The deposit was recorded but the sweep failed.", {
                    userWalletId,
                    amount,
                    error: errorMessage,
                });
                ...
                console.error("[CircleWallet] Failed to sweep USDC to hot wallet:", errorMessage);
                if (error.response) {
...
exports.initializeCircleWallet = initializeCircleWallet;
const getCircleWallet = () => {
    return circleWalletService;
};
exports.getCircleWallet = getCircleWallet;
exports.default = circleWalletService;

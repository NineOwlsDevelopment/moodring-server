...
exports.calculateEvidenceHash = calculateEvidenceHash;
exports.verifyEvidenceSignature = verifyEvidenceSignature;
exports.requiresMultiAdminApproval = requiresMultiAdminApproval;
 ...
 */
function validateEvidence(evidence, resolutionMode) {
    if (!evidence) {
    ...
    if (resolutionMode === "ORACLE") {
        if (evidenceData.source === "manual") {
            return {
                isValid: false,
                error: "ORACLE mode requires cryptographic proof (chainlink, onchain, or signed API)",
            };
        }
 ...
 */
function calculateEvidenceHash(evidence) {
    const evidenceString = JSON.stringify(evidence, Object.keys(evidence).sort());
    return (0, crypto_1.createHash)("sha256").update(evidenceString).digest("hex");
}
/**
 ...
 */
function requiresMultiAdminApproval(marketVolume, threshold = 100000000 * 10 ** 6) {

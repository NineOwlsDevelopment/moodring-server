...
class TradeModel {
    static async create(data, client) {
        const { user_id, market_id, option_id, trade_type, side, quantity, price_per_share, total_cost, fees_paid = 0, transaction_signature = null, status = "completed", } = data;
    ...
    static async findByMarket(marketId, limit = 50, offset = 0, client) {
        const db = client || db_1.pool;
        const [tradesResult, countResult] = await Promise.all([
            db.query(`
      ...
      `, [marketId, limit, offset]),
            db.query("SELECT COUNT(*)::int as count FROM trades WHERE market_id = $1", [marketId]),
        ]);
        return {
            trades: tradesResult.rows,
            total: countResult.rows[0]?.count || 0,
    ...
    static async findByOption(optionId, limit = 50, offset = 0, client) {
        const db = client || db_1.pool;
        const [tradesResult, countResult] = await Promise.all([
            db.query(`
      ...
      `, [optionId, limit, offset]),
            db.query("SELECT COUNT(*)::int as count FROM trades WHERE option_id = $1", [optionId]),
        ]);
        return {
            trades: tradesResult.rows,
            total: countResult.rows[0]?.count || 0,
    ...
    static async getRecentTrades(limit = 20, client) {
        const db = client || db_1.pool;
    ...
    static async getUserTradeStats(userId, client) {
        const db = client || db_1.pool;
        const result = await db.query(`
      SELECT 
    ...
    static async updateStatus(id, status, transactionSignature, client) {
        const db = client || db_1.pool;
        const result = await db.query(`
      UPDATE trades

...
class CommentModel {
    ...
    static async findByMarket(marketId, currentUserId, limit = 50, offset = 0, sort = "new", client) {
        const db = client || db_1.pool;
      ...
      `, [marketId, limit, offset, currentUserId || null]),
            db.query("SELECT COUNT(*)::int as count FROM comments WHERE market_id = $1 AND parent_id IS NULL AND is_deleted = FALSE", [marketId]),
        ]);
        // Ensure counts are numbers
    ...
    static async update(id, userId, content, client) {
        const db = client || db_1.pool;
        const result = await db.query(`
      UPDATE comments
    ...
    static async softDelete(id, userId, client) {
        const db = client || db_1.pool;
        const result = await db.query(`
      UPDATE comments
    ...
    static async vote(commentId, userId, voteType) {
        ...
        try {
            ...
            const existingVote = await client.query("SELECT vote_type FROM comment_votes WHERE comment_id = $1 AND user_id = $2", [commentId, userId]);
            if (existingVote.rows.length > 0) {
                ...
                if (oldVoteType === voteType) {
                    ...
                    await client.query(`UPDATE comments SET ${voteType === "up" ? "upvotes" : "downvotes"} = ${voteType === "up" ? "upvotes" : "downvotes"} - 1 WHERE id = $1`, [commentId]);
                }
                else {
                    // Change vote
                    await client.query("UPDATE comment_votes SET vote_type = $1 WHERE comment_id = $2 AND user_id = $3", [voteType, commentId, userId]);
                    await client.query(`UPDATE comments SET 
            ...
            WHERE id = $1`, [commentId]);
                ...
                // New vote
                const now = Math.floor(Date.now() / 1000);
                await client.query("INSERT INTO comment_votes (comment_id, user_id, vote_type, created_at) VALUES ($1, $2, $3, $4)", [commentId, userId, voteType, now]);
                await client.query(`UPDATE comments SET ${voteType === "up" ? "upvotes" : "downvotes"} = ${voteType === "up" ? "upvotes" : "downvotes"} + 1 WHERE id = $1`, [commentId]);
            }
            // Get updated counts
            const result = await client.query("SELECT upvotes, downvotes FROM comments WHERE id = $1", [commentId]);
    ...
    static async getUserCommentCount(userId, client) {
        const db = client || db_1.pool;
        const result = await db.query("SELECT COUNT(*)::int as count FROM comments WHERE user_id = $1 AND is_deleted = FALSE", [userId]);

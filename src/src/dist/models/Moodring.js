...
exports.MoodringAdminModel = exports.MoodringModel = void 0;
...
class MoodringModel {
     ...
     */
    static async initialize(data, client) {
        const db = client || db_1.pool;
     ...
     */
    static async incrementTotalMarkets(client) {
        const db = client || db_1.pool;
     ...
     */
    static async recordFees(creatorFees, protocolFees, client) {
        const db = client || db_1.pool;
        const result = await db.query(`
      UPDATE moodring
      SET 
     ...
     */
    static async recordProtocolFeeWithdrawal(amount, client) {
        const db = client || db_1.pool;
        const result = await db.query(`
      UPDATE moodring
      SET 
     ...
     */
    static async updateStats(tvlDelta, volumeDelta, client) {
        const db = client || db_1.pool;
        const result = await db.query(`
      UPDATE moodring
      SET 
     ...
     * Get trading limits
     */
    static async getTradingLimits(client) {
        const db = client || db_1.pool;
        const result = await db.query(`
      SELECT
        min_trade_amount,
     ...
     * Get market controls
     */
    static async getMarketControls(client) {
        const db = client || db_1.pool;
        const result = await db.query(`
      SELECT
        max_markets_per_user,
     ...
     */
    static async getResolutionControls(client) {
        const db = client || db_1.pool;
        const result = await db.query(`
      SELECT
        auto_resolve_markets,
     ...
     * Get risk controls
     */
    static async getRiskControls(client) {
        const db = client || db_1.pool;
        const result = await db.query(`
      SELECT
        max_market_volatility_threshold,
     ...
     */
    static async getDisputeControls(client) {
        const db = client || db_1.pool;
        const result = await db.query(`
      SELECT
        default_dispute_period_hours,
     ...
     * Get feature flags
     */
    static async getFeatureFlags(client) {
        const db = client || db_1.pool;
        const result = await db.query(`
      SELECT
        enable_copy_trading,
     ...
     */
    static async isFeatureEnabled(feature, client) {
        const featureMap = {
        ...
        const db = client || db_1.pool;
        const result = await db.query(`SELECT ${featureMap[feature]} FROM moodring LIMIT 1`);
        return result.rows[0]?.[featureMap[feature]] || false;
    }
    /**
     * Check if platform allows a specific action
     */
    static async isActionAllowed(action, client) {
        const actionMap = {
        ...
        const db = client || db_1.pool;
        const result = await db.query(`SELECT ${actionMap[action]} FROM moodring LIMIT 1`);
        return result.rows[0]?.[actionMap[action]] || false;
    }
}
exports.MoodringModel = MoodringModel;
/**
 * Model for managing moodring admins (separate table)
 */
class MoodringAdminModel {
    /**
     * Get all admins
     */
    static async getAll(client) {
        const db = client || db_1.pool;
        const result = await db.query("SELECT * FROM moodring_admins ORDER BY created_at ASC");
        return result.rows;
    }
    /**
     * Get all admin user IDs
     */
    static async getAdminUserIds(client) {
        const db = client || db_1.pool;
     ...
     */
    static async isAdmin(userId, client) {
        const db = client || db_1.pool;
        const result = await db.query("SELECT 1 FROM moodring_admins WHERE user_id = $1 LIMIT 1", [userId]);
        return result.rows.length > 0;
    }
    /**
     * Add an admin
     */
    static async addAdmin(userId, client) {
        const db = client || db_1.pool;
        // Check if already admin
        const existing = await db.query("SELECT * FROM moodring_admins WHERE user_id = $1", [userId]);
        if (existing.rows.length > 0) {
            return existing.rows[0];
        }
        const result = await db.query("INSERT INTO moodring_admins (user_id) VALUES ($1) RETURNING *", [userId]);
        return result.rows[0] || null;
    }
    /**
     * Remove an admin
     */
    static async removeAdmin(userId, client) {
        const db = client || db_1.pool;
        const result = await db.query("DELETE FROM moodring_admins WHERE user_id = $1 RETURNING id", [userId]);
        return result.rows.length > 0;
    }
    /**
     * Get admin by user ID with user details
     */
    static async getAdminWithUser(userId, client) {
        const db = client || db_1.pool;
        const result = await db.query(`
     ...
     */
    static async getAllWithUsers(client) {
        const db = client || db_1.pool;
        const result = await db.query(`
      SELECT ma.*, u.username, u.email
      FROM moodring_admins ma
...
exports.MoodringAdminModel = MoodringAdminModel;
//# sourceMappingURL=Moodring.js.map
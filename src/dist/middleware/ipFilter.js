...
exports.adminIPWhitelist = exports.globalIPBlacklist = void 0;
 ...
 */
const globalIPBlacklist = (req, res, next) => {
    ...
    const filter = (0, express_ipfilter_1.IpFilter)(blacklist, {
        mode: "deny",
        log: false,
        logLevel: "deny",
    ...
    return filter(req, res, (err) => {
        if (err instanceof express_ipfilter_1.IpDeniedError) {
            ...
            console.warn(`[IP Filter] Blocked request from IP: ${req.ip}${cfIP ? ` (CF-Connecting-IP: ${cfIP})` : ""}`);
...
exports.globalIPBlacklist = globalIPBlacklist;
/**
...
const adminIPWhitelist = (req, res, next) => {
    ...
    if (whitelist.length === 0) {
        if (process.env.NODE_ENV === "production") {
            console.error("❌ ADMIN_IP_WHITELIST not set in production. BLOCKING all admin access.");
            return res.status(503).json({
                error: "Service Unavailable",
                message: "Admin access is not configured. Contact system administrator.",
            });
        }
        // Development: allow with warning
        console.warn("⚠️  ADMIN_IP_WHITELIST not set. Allowing all IPs in development.");
        return next();
    }
    ...
    const cfIP = req.headers["cf-connecting-ip"];
    const clientIP = cfIP?.trim() || req.ip || req.socket.remoteAddress || "unknown";
    // Validate IP format
    ...
    const allowedIPs = process.env.NODE_ENV !== "production"
        ? [...whitelist, "127.0.0.1", "::1", "::ffff:127.0.0.1", "localhost"]
        : whitelist;
    // Check if client IP is in whitelist
    if (!allowedIPs.includes(clientIP)) {
        const connectionIP = req.socket.remoteAddress || req.ip || "unknown";
        console.warn(`[IP Filter] Blocked admin request from IP: ${clientIP}${cfIP ? ` (CF-Connecting-IP: ${cfIP})` : ""} (connection IP: ${connectionIP})`);
        return res.status(403).json({
            error: "Forbidden",

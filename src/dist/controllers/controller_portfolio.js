...
exports.getPnLSummary = exports.getLiquidityPositions = exports.getPositions = exports.getPortfolio = void 0;
...
const getPortfolio = async (req, res) => {
    try {
        const userId = req.id;
        // Get wallet balance
        const wallet = await Wallet_1.WalletModel.findByUserId(userId);
        if (!wallet) {
            return (0, errors_1.sendNotFound)(res, "Wallet");
        }
        // Get user positions across all markets
        const positionsResult = await db_1.pool.query(`
      SELECT 
        up.*,
    ...
    `, [userId]);
        ...
        const liquidityResult = await db_1.pool.query(`
      SELECT 
        lp.*,
        m.question as market_question,
        m.image_url as market_image,
    ...
    `, [userId]);
        // Get user stats
        const stats = await UserStats_1.UserStatsModel.findByUserId(userId);
        // Get trade summary
        const tradeSummary = await Trade_1.TradeModel.getUserTradeStats(userId);
        ...
        for (const lp of liquidityPositions) {
            if (lp.market_resolved) {
                ...
                if (lp.total_shared_lp_shares &&
                    ...
                    const shareValue = Math.floor((userShares * (poolLiquidity + accumulatedFees)) / totalShares);
...
const getPositions = async (req, res) => {
    try {
        ...
        const status = req.query.status; // 'active', 'resolved', or undefined for all
        let whereClause = "WHERE up.user_id = $1 AND (up.yes_shares > 0 OR up.no_shares > 0)";
        if (status === "active") {
      ...
      `, [userId]),
        ...
        const transformedPositions = positionsResult.rows.map((pos) => {
            ...
            return {
                ...pos,
                side,
                shares: shares / 1000000, // Convert from micro-shares to display units
                avg_price: avgPrice / 1000000, // Convert from micro-USDC to decimal (0-1 range)
                current_price: currentPrice,
                pnl: pnl / 1000000, // Convert from micro-USDC to USDC
 ...
 * @access Private
 */
const getLiquidityPositions = async (req, res) => {
    try {
        const userId = req.id;
        const page = parseInt(req.query.page) || 1;
        ...
        LIMIT $2 OFFSET $3
      `, [userId, limit, offset]),
            db_1.pool.query("SELECT COUNT(*)::int as count FROM lp_positions WHERE user_id = $1 AND shares > 0", [userId]),
        ]);
        const total = countResult.rows[0]?.count || 0;
        const totalPages = Math.ceil(total / limit);
        // Transform positions to match frontend interface
...
const getPnLSummary = async (req, res) => {
    try {
        ...
        const tradesResult = await db_1.pool.query(`
      SELECT 
        trade_type,
        side,
    ...
    `, [userId]);
        ...
        let unrealizedPnL = 0;
        const openPositions = positions.filter((p) => !p.market_resolved &&
...
//# sourceMappingURL=controller_portfolio.js.map